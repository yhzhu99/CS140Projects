...
由于Pintos的内核不支持浮点数这一功能。然而，在实验中，计算优先级需要用到`recent_cpu`和`last_cpu`等数值，而这些数值以及与这些数值有关的多种计算的中间结果和最终结果，很大可能性不是一个整数，必须用浮点数来进行计算和表示。在这种情况下，需求和功能之间的不匹配或多或少产生了一些不可避免也不可忽视的矛盾。
