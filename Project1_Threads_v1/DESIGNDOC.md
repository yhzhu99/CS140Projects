...
以上根据priority-donate-one等简单测试点得出的需求。在更为复杂，更为综合的情景中，需要考虑更多的需求。比如，在线程释放掉锁之后，线程需要重新考虑自身的权重，或是恢复为原本的权重，或是重新从未释放的锁中获取当前最高的权重。此外，在例如priority-donate-chain和multiple等                                                                                                      
